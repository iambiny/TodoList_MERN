...
todosRouter.post('/', authenticate, async (req: Request & { user?: UserPayLoadType }, res) => {
...
todosRouter.post('/create', authenticate, async (req: Request & { user?: UserPayLoadType }, res) => {
    const userId = req.user?.userId;
    try {
